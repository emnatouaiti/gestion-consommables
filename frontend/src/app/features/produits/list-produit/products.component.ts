...
export class ProductsComponent implements OnInit {
  ...
  generateDescriptions(): void {
    const title = this.productForm.get('title')?.value;
    if (!title || !title.trim()) {
      this.errorMessage = 'Titre requis pour g̩n̩rer la description.';
      return;
    }
    this.errorMessage = '';
    const payload = {
    ...
    this.stockService.generateDescriptions(payload).subscribe({
      ...
      error: (err: any) => {
        this.errorMessage = this.extractApiError(err, 'Impossible de g̩n̩rer la description.');
      }
    });
  }
  ...
  loadAll(): void {
    ...
    this.stockService.listCategories({ tree: true }).subscribe({
      ...
      error: (err: any) => {
        this.errorMessage = err?.message || 'Erreur de chargement des cat̩gories.';
  ...
  save(): void {
    ...
    if (this.editingId && (!payload.reference || payload.reference === '')) {
      this.errorMessage = 'R̩f̩rence est obligatoire lors de la modification.';
      return;
    }
    this.errorMessage = '';
    ...
    req$.subscribe({
      next: (res: any) => {
        ...
        if (this.editingId) {
          ...
          this.successMessage = 'Produit cr̩̩ ! R̩f: ' + created.reference;
          this.highlightedProductId = created.id || null;
        } else {
          this.successMessage = 'Produit cr̩̩ !';
        }
        this.closeModal();
        this.loadProducts();
  ...
  confirmReactivate(): void {
    ...
    this.stockService.activateProduct(this.reactivateProduct.id).subscribe({
      next: () => {
        this.successMessage = `Produit "${this.reactivateProduct.title}" r̩activ̩ !`;
      ...
      error: (err: any) => {
        this.errorMessage = this.extractApiError(err, 'Impossible de r̩activer le produit.');
      }
    });
  }
