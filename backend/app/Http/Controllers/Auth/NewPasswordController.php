    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if (!$resetRecord) {
            return back()->withInput($request->only('email'))
                ->withErrors(['code' => 'Code invalide ou expirÃƒA.']);
        }
        ...
        if ($resetRecord->created_at && now()->diffInMinutes($resetRecord->created_at) > 15) {
            ...
            return back()->withInput($request->only('email'))
                ->withErrors(['code' => 'Ce code a expirÃƒA. Veuillez demander un nouveau code.']);
        }
        ...
        return redirect()->route('dashboard')->with('status', 'Mot de passe rÃƒAinitialisÃƒA avec succÃƒA¨s');
    }
}
