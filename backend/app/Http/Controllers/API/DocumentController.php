...
class DocumentController extends Controller
{
    ...
    private function inferTitle(string $ocrText, ?string $guessedType, string $fallbackName, ?string $userTitle): string
    {
        ...
        if (str_contains($text, 'bon de reception') || str_contains($text, 'bon de réception')) return 'Bon de réception';
        if (str_contains($text, 'bon de sortie')) return 'Bon de sortie';
        if (str_contains($text, 'réception de marchandise') || str_contains($text, 'reception de marchandise')) return 'Bon de réception';
    ...
    private function isLikelyValidTitle(string $title): bool
    {
        ...
        if (!preg_match('/[a-zàâäéèêëîïôöùûüç]/iu', $t)) return false;
        ...
        $symbols = preg_match_all('/[^a-z0-9àâäéèêëîïôöùûüç\s\-]/iu', $t);
        if ($symbols !== false && $symbols > 3) return false;

        // Very short vowel-less gibberish
        if (mb_strlen($t) <= 8 && !preg_match('/[aeiouyàâäéèêëîïôöùûü]/iu', $t)) return false;
     ...
     *  - Year guard covers 1900–2199
     *  - knownQtyMisreads expanded with 'oe'=>40, 'ea'=>80, 'go'=>60
     *  - Fuzzy substitution only on tokens ≤ 3 chars
     */
    private function tryParseQuantity(string $token): ?int
    {
        ...
        // 4. Fuzzy substitution only for very short tokens (≤ 3 chars)
    ...
    private function isHeaderRow(string $rowText): bool
    {
        ...
        if (preg_match('/r[eéiî]?f[eéiî]?[rn]?[eéiî]?[nc]?[ec]?[eo]?|rif/i', $t)) $signals++;
        ...
        if (preg_match('/quan?ti?[tdé]?[eé]?|qte|qty|ouanti|uanti/i', $t)) $signals++;
    ...
    private function parseLinesFromTSV(string $tsv, ?string $storedPath = null): array
    {
        ...
        // Locate the header row (Reference + Quantité columns)
        $headerRowIdx  = null;
        $refColEnd     = null;
        $qtyColStart   = null;
        $qtyColEnd     = null;
        $qtyPrimaryStart = null; // "Quantité Livrée" when present
        $qtyPrimaryEnd   = null;
        $titleColStart = null;
        $titleColEnd   = null;
        ...
        foreach ($rows as $idx => $row) {
            ...
            $hasRef   = preg_match('/r[eéiî]?f/', $rowText) === 1;
            $hasQty   = preg_match('/quan?ti?[tdé]?[eé]?|qty|qte/', $rowText) === 1;
            $hasDescr = preg_match('/descri?p?ti?[o0]?n?|produit/', $rowText) === 1;

            if ($hasRef && ($hasQty || $hasDescr)) {
                $headerRowIdx = $idx;
                foreach ($row as $word) {
                    if (preg_match('/r[eéiî]?f[eéiî]?[rn]?[eéiî]?[nc]?[ec]?[eo]?/i', $word['text'])) {
                        $refColEnd = $word['right'] + 80;
                    }
                    if (preg_match('/quan?ti?[tdé]?[eé]?/i', $word['text'])) {
                    ...
                    if (preg_match('/livr[ée]e?/i', $word['text'])) {
        ...
        foreach (array_slice($rows, $headerRowIdx + 1) as $row) {
            ...
            foreach ($refTokens as $w) {
                ...
                // 4-digit number only → treat as INV sequence
                if (preg_match('/^(\d{4})$/', $tok, $m)) {
                    $ref = 'INV-' . $m[1];
                    break;
                }
                // Generic numeric row references (1, 2, 3...) for table formats with "Réf."
            ...
            if ($qty === null) {
                // Fallback: scan individual tokens right → left
            ...
            // If document has two quantity columns (commandée/livrée), prefer right-most (livrée).
            if (!empty($qtyTokens)) {
            ...
            // STEP 2: fallback — scan title tokens right → left
    ...
    private function parseLines(string $text): array
    {
        ...
        foreach ($lines as $line) {
            ...
            if (preg_match('/bon de livraison|numero de bon|référence n/i', $lowLine)) continue;
    ...
    private function extractTableFromImageHeuristic(string $fullPath): array
    {
        ...
        try {
            ...
            foreach ($lines as $line) {
                ...
                if ($low === '') continue;
                if (preg_match('/réf|ref|désignation|quantité|commandée|livrée|livree/i', $low)) continue;
                if (preg_match('/client|adresse|date|bon|livraison|recu|regu|email|gmail/i', $low)) continue;

                $clean = preg_replace('/\s+/', ' ', trim($line));

                // Pattern: "1 gel à main 20 20" -> qty = last (livrée)
    ...
    private function guessSupplierEmail(string $text): ?string
    {
        ...
        foreach (explode("\n", $text) as $line) {
            ...
            if (str_contains($lower, 'contact') || str_contains($lower, 'email') || str_contains($lower, 'mail')) {
                // Match pattern like "contacttechpro.tn" → try inserting @ before the domain
    ...
    {
        $t = Str::lower($text);
        if (preg_match('/bon\s*de\s*livraison|bondelivraison|r[eé]ception|entree|entr[eé]/iu', $t)) return 'in';
    ...
    public function findAvailableLocation(Request $request)
    {
        ...
        if (!$warehouseId) return response()->json(['message' => 'Aucun dépôt trouvé pour cet utilisateur.'], 404);
